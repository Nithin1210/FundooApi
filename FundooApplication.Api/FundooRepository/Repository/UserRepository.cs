...
namespace FundooRepository.Repository
{
    public class UserRepository : IUserRepository
    {
        ...
        public Register ResetPassword(string email0,string newPassword,string confirmPassword)
        {
            try
            {
                if (newPassword.Equals(confirmPassword))
                {
                    var input = this.context.Register.Where(x => x.Email.Equals(email0)).FirstOrDefault();
                    if (input != null)
                    {
